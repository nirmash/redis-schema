...
```
172.28.1.4:6379> schema.number_rule model_year_ 1940 2023
OK
```
...
```
172.28.1.4:6379> schema.upsert_row -1 contacts firstName john lastName doe age 25
(integer) 3
```
...
```
172.28.1.4:6379> schema.upsert_row 1 contacts age 35
(integer) 0
```
...
172.28.1.4:6379> schema.upsert_row -1 contacts firstName john lastName doe age 25
(integer) 3
172.28.1.4:6379> schema.upsert_row -1 contacts firstName jane lastName doe age 30
(integer) 3
172.28.1.4:6379> schema.upsert_row -1 contacts firstName alexander lastName hamilton age 45
(integer) 3
172.28.1.4:6379> schema.upsert_row -1 contacts firstName george lastName washington age 50
(integer) 3
172.28.1.4:6379> schema.upsert_row -1 contacts firstName ben lastName smith age 11
(integer) 3
```
...
```
Now let's load the select all script for our table. The `lua_publish.py` scripts takes 3 parameters. It takes the name of the Lua file we want to load. 
```
root@9272b3492e54:/src# python3 lua_publish.py select_all.lua
...
```
172.28.1.4:6379> schema.execute_query_lua select_all.lua contacts
1) 1) "Id"
   2) "3"
   3) "firstName"
   4) "alexander"
